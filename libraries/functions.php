...
function check_permission_access($value,$string){
	if (strpos($string, $value) !== false) {
	    return true;
	}
	return false;
}
function check_permission($value,$string){
	if (strpos($string, $value) !== false) {
...
function get_permission($id){
	...
	$d->reset();
    $d->query("select permission from #_phanquyen where id='".$id."'"); 
    $kq = $d->fetch_array();
    return $kq['permission'];
}
function get_name_quyen($id){
	global $d;
    $d->query("select ten from #_phanquyen where id=$id");$kq=$d->fetch_array();return $kq['ten'];
}
function hidden_menu($value,$string){
	global $_SESSION;
	$check = true;
	if($_SESSION['login']['role']==1){
		$arr_value = explode(',',$string);
...
function sub_str($chuoi,$gioihan){
